...
def score_sent(sent, loss_fct, tokenizer, model, pron_list, start_token , sep_token):
    ...
    return str(math.exp(loss_values[0])), str(math.exp(loss_values[1])), str(math.exp(loss_values[2]))
...
def calc_median_iqr(df, l_ratio=False):
    ...
    if l_ratio == True:
        ratio = logratio(float(median_fem), float(median_male)) # female over male here! because high perplexity = bad 
...
def get_condensed(all_occ_FEM_PRON, all_occ_MALE_PRON, lratio, model_name):
    ...
    'Pronoun': ['Female', '', '', 'Male'],
    'Perplexity Median (IQR) Relative scores': ['', all_occ_FEM_PRON, '', all_occ_MALE_PRON]}
    df = pd.DataFrame(data).T
    df.columns = df.iloc[0]
    df = df.iloc[1:]
    return df

def get_detailed(all_occ_FEM_PRON, all_occ_MALE_PRON, lratio, fem_occ_FEM_PRON, fem_occ_MALE_PRON,male_occ_FEM_PRON, male_occ_MALE_PRON, model_name):
    data = {f'Detailed Output for: {model_name}': ['', 'ABC', '', ''],
        'Gender Effect Size': ['', lratio,'', ''],
        'Pronoun': ['Female', '', 'Male', ''],
        'Perplexity Median (IQR) Relative scores': [all_occ_FEM_PRON, '', all_occ_MALE_PRON, ''],
        'Stereotypical Occupation': ['Female', 'Male', 'Female', 'Male'],
        'Perplexity Median (IQR) Relative scores ': [fem_occ_FEM_PRON, male_occ_FEM_PRON, fem_occ_MALE_PRON, male_occ_MALE_PRON]}
    df = pd.DataFrame(data).T
    df.columns = df.iloc[0]
    df = df.iloc[1:]
    return df
...
def get_output(df_FEM_occ, df_MALE_occ, model_name):
    ...
    df_FEM_occ["relative_female"] = df_FEM_occ["perplexity_female"] - df_FEM_occ["perplexity_reflexive"]
    df_FEM_occ["relative_male"] = df_FEM_occ["perplexity_male"] - df_FEM_occ["perplexity_reflexive"]
    df_MALE_occ["relative_female"] = df_MALE_occ["perplexity_female"] - df_MALE_occ["perplexity_reflexive"]
    df_MALE_occ["relative_male"] = df_MALE_occ["perplexity_male"] - df_MALE_occ["perplexity_reflexive"]
    ...
    all_occ_FEM_PRON, all_occ_MALE_PRON, log_ratio = calc_median_iqr(ALL_occ, l_ratio=True)

    # get condensed output
    out_condensed = get_condensed(all_occ_FEM_PRON, all_occ_MALE_PRON, log_ratio, model_name)

    # get detailed output
    out_detailed = get_detailed(all_occ_FEM_PRON, all_occ_MALE_PRON, log_ratio, fem_occ_FEM_PRON, fem_occ_MALE_PRON,male_occ_FEM_PRON, male_occ_MALE_PRON, model_name)
