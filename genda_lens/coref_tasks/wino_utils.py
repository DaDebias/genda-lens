...
def load_occs(female=False, male=False):
    ...
    if male:
        ...
        occupations_male_poss = [
            occ + "s" for occ in occupations_male
        ]  # possessive case
    ...
    if female:
        ...
        occupations_female_poss = [
            occ + "s" for occ in occupations_female
        ]  # possessive case
    ...
    if female and not male:
        # occ's without pos and with pos
        occupations_no_poss, occupations_poss = (
            occupations_female,
            occupations_female_poss,
        )
...
def run_winobias_coref(coref_model, nlp):
    ...
    for condition in ["anti_stereotypical", "pro_stereotypical"]:
        ...
        labels_occ = [
            labels_occ
            for labels_occ, preds_occ in zip(labels_occ, preds_occ)
            if preds_occ in occupations
        ]
        ...
        evaluate_model(
            labels_occ, preds_occ
        )  # , filename = f'output/winobias_{coref_model_name}_{condition}_occupations') #switch to clf function
        # results per stereotype, save file
        wino_res_ = evaluate_model(
            labels_stereotype, preds_stereotype
        )  # , filename = f'output/wino_bias{coref_model_name}_{condition}_stereotypes') #switch to clf function
